...
RUN { \
    ...
    cd ../host-build && \
    tar xf ../src/qtbase-everywhere-src-6.10.2.tar.xz && \
    tar xf ../src/qtshadertools-everywhere-src-6.10.2.tar.xz && \
    tar xf ../src/qtdeclarative-everywhere-src-6.10.2.tar.xz && \
    ...
    cd ../qtshadertools-everywhere-src-6.10.2 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile declerative for host" && \
    cd ../qtdeclarative-everywhere-src-6.10.2 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    cd ../../pi-build && \
